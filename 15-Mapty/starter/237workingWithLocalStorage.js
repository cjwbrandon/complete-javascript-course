...
class App {
  ...
  _loadMap(position) {
    ...
    this.#workouts.forEach(work => {
      // Must wait for map to load before executing
  ...
  _newWorkout(e) {
    ...
    if (type === 'running') {
      ...
      // Check if data is valid -> Input Validation
      if (
  ...
  _moveToPopup(e) {
    const workoutEl = e.target.closes('.workout');
  ...
  _setLocalStorage() {
    ...
    // Do not use localStorage for big amount of data -> Blocking -> poor performance
  }
...
// 1 issue with localStorage
// Prototype chain is gone
// When convert to string and back -> Prototype chain is gone
